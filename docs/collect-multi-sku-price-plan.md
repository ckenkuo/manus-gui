...
## 8. 已落地：排除已作废规格（2026-09-17）

- 商品采集按 SKU 的申报价状态过滤，不按 SPU 整体排除，也不把「已加入站点数为 0」
  或商品上下架状态当作作废依据。同一商品的生效规格仍可采集。
- 状态码已核对 Temu 页面自身的枚举：`CheckPriceReject=3` 对应「已作废」，
  `CheckPricePass=2` 对应「已生效」。核对来源为公开前端资源
  `https://bstatic.kwcdn.com/static/temu-sca-newon/product-select-6b882012.7a4f5be5.chunk.js`，
  文案在 `https://bstatic.kwcdn.com/static/temu-sca-newon/main.f327cb0d.js`。
- 枚举脚本优先读取与 SKU、报价站点匹配的 `supplierPriceReviewInfoList[].status`
  （与页面显示口径一致），再退到站点 `priceReviewStatus`、SKU `priceReviewStatus`。
  写入清单字段为 `price_review_status`，不能用真假值判断丢掉状态 0。
- 清单文件保留带状态的完整快照；读取时先排除作废规格，再按 SPU 合并规格。
  待采统计、枚举返回数量和实际写表共用这一口径。全部作废时可采清单为空，
  不会退回 SPU 行，也不会因「空结果保护」继续使用旧的生效清单。
- 批次入口再次排除作废规格。不含状态字段的旧清单禁止直接采集，提示先重新枚举；
  状态取自最近一次枚举，并非采集过程中持续轮询。后台状态改变后应重新枚举。
  接口确实未返回状态时保留 `null`，不臆断为作废；本改动不要求其它状态必须已生效。
- 不清理已写入成本表的历史数据。升级后重启服务，在已登录的商品列表页重新枚举再采集。

## 9. 当前成本表判重口径：只按 SPU ID

- 2026-09-17 用户明确要求：不再把 SKU ID、规格名称或价格作为重复条件。
- 本地和云端目标表均只读取真实表头对应的 SPU 列。只要该 SPU 已有任意一行，
  就跳过该商品全部规格，不再补录另一个规格；规格改名、价格变化也不新增行。
- 新商品先排除已作废规格，再按店铺、区域和 SPU 保留平台返回的首个可采规格。
  一批写入同一目标表时再用纯 SPU 键拦截重复条目，保证每个新 SPU 最多写一行。
  因此成本表不会完整记录同 SPU 的所有规格价格，代表行的规格和价格仍来自同一 SKU。
- 原始清单保留全部 SKU 和状态，便于审计；待采统计和实际采集使用合并后的清单。
  已有的作废过滤及旧清单重新枚举要求不变。不修改订单登记表的判重，也不删除历史成本行。
- 店铺身份从完整清单快照识别，可采商品另外过滤。某店全部规格作废时，店铺下拉仍保留
  该店并显示 0 件；使用该店的保存偏好或显式选择采集时直接结束，不触发失配回退、
  不采集其他店铺或区域，也不更换目标表。
