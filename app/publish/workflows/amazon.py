"""亚马逊来源的兼容流程，缺乏成分证据时不使用 1688 属性规则。"""

from .base import Stage, product_fields

AGE_ATTR_KEYS = ("Age Range", "Age Range Description", "适用年龄")
...
async def publish_one(session, task, store, **kwargs):
    ...
    return await get_workflow("amazon").publish_one(session, task, store, **kwargs)


async def run_batch(tasks, **kwargs):
    from . import get_workflow

    return await get_workflow("amazon").run_batch(tasks, **kwargs)
...
def build_stages():
    ...
    return (
        Stage("extract", "① 亚马逊采集提炼", extracting._st_extract),
        Stage("claim", "② 亚马逊商品认领", form._st_claim),
