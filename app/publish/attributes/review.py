...
6. 里料纹理/里衬类字段有联动必填：选「光面」「绒面/PU」等会动态新增必填行
   ...
   含"针织""毛衣"时织造方式应为针织类。
6b.【禁词选项一律避开，选同义的合规项】options 里若出现「pp棉」这类带平台禁词的值，
   不要选它——改选语义相同但不含禁词的写法（「聚丙烯纤维/丙纶」或「聚酯纤维」，
   PP 棉的化学名就是聚丙烯，填的是同一种材料）。源商品参数里写着「填充物：pp棉」时
   同理：按材料本身选合规选项，不要照抄源写法。「安抚」类用途描述同属禁词，
   options 里遇到一并避开。整行 options 全是禁词、实在无合规项可选时才保持原值并在
   notes 里记一句。
