...
5. **取消勾选序号 4「收货信息」**——登记表用不到，且是买家 PII（姓名/电话/邮箱/身份证号/税号/地址），
   ...
   **坑一（点不动）**：这些复选框和表头全选框是同一套 beast-core 组件，结构是
   `<label data-testid=beast-core-checkbox><input 隐形><span>文案</span></label>`——真实
   `<input>` 是 0×0 + `opacity:0`，**点它必然超时报 `element is not visible`**。
   必须【状态读 input、点击点 label】。2026-08-06 实机踩到：增量早停后走到导出，卡在这个
   checkbox 上 30s 超时，异常一路抛到 service 把整批中止，前面翻完的页和勾好的单全部作废。
   修法两条：点 label（根因），且整段取消勾选 **best-effort 吞异常**——它是可跳过的隐私收敛，
   点不动只该告警提醒人核对导出列，绝不该让一批白跑。click 超时也从默认 30s 压到 5s。

   **坑二（点错了别人）**：定位**必须用「自身带该文案的 checkbox label」**
   （`label[data-testid=beast-core-checkbox]:has-text("收货信息")`），
   **绝不能**写成 `label:has-text("收货信息")` 再在其后代里取 `.first`——后者会匹配到包着
   整组的外层 label，`.first` 点到的是**导出范围开关**。2026-08-06 修坑一时就这么翻了车：
   增量早停只勾了 20 单，导出却是整页 182 单，**日志全绿、导出内容全错**，比超时报错难查得多。
   文案和 `data-testid` 本就在同一个 label 上，精确定位不需要任何猜测。
   另加一道 `_warn_if_export_scope_is_all()` 护栏：确认前若检出范围是「全部」就大声告警
   （只告警不中止——这个控件的 DOM 未实测确认，硬当判据会误拦正常批次；要收紧得先按
   1688 选择器那套把结构实测记档）。
...
**判重键**：`订单号 + 尺码`（默认）。**绝不能只用订单号**——一个订单可含多个子订单（多行），
只比订单号会把除第一行外的兄弟行全判成重复丢掉，那些行从没写过，等于**少买**。

### 5.1 判重键按表自适应（2026-08-06）

起因：实机 config 配成了 `dedupe_by = ["订单号"]`，而同一订单的两个子订单在同批内撞键，
第二行被静默丢弃。同时发现尺码这一位本身也不可靠——它取自导出的「商品属性」，实测有的 SKU
属性**恒为字符串 `Variant`**（Temu 那边就没有规格名），同订单里两个该商品的子订单照样撞键。
真正一单一号的是**子订单号**，但各表加这一列的进度不一样（2026-08-06 实测云端 4 张表都还
没加，本地表刚加了一张）。

所以判重键不写死，改成**逐表自适应**（`_preferred_dedupe_by`，按 Sheet 解析而非全批共用）：

- 配置里已指定区分位（`子订单号` / `尺码` 之一）→ 尊重配置，不动。
- 只配了 `["订单号"]` 这类单列键 → 看表里有哪列可用，自动补一位：**优先子订单号**，其次尺码；
  两者都没有则维持原样（风险由下面的 `collided` 暴露）。补列会打 INFO 日志。
- 默认值仍是 `["订单号", "尺码"]`：把子订单号设成默认会让缺列的表 `no_key` 整表跳过、一行
  都写不进，比少买更糟。

同时把两种 dup 分开报，不再混成一个数字：

| 字段 | 含义 |
|------|------|
| `dup_skipped` | 总跳过数（表里已有 + 同批撞键） |
| `collided_skipped` | 其中**同批撞键**的行数 —— 这些行从没写过就被丢了，等于少买，必须人工过问 |

`collided` 判定靠「进本批前表里已有的键」的快照（`existing`）：命中 `seen` 但不在快照里，
就是被本批前面某行占了位。`plan` 事件里带 `dedupe_by`（该表实际用的列）与 `collided_samples`，
`_stage_order` 里撞键还会打 WARNING 并建议加子订单号列。

管线现在也会往「子订单号」列写值（`_FIELD_SOURCES` 新增），所以加了列就能直接用。
另外 `run_orders_batch` 检测到 `dedupe_by` 只有订单号一位时会告警——那是静默少买的配置。
...
### 16.2 早停页改为「只勾水位之前的行」（2026-08-06）

初版在早停那一页走**整页全选**，理由是「订单号已登记不代表它每个尺码都登记了，重叠旧单
交给判重挡掉，代价只是多导出几十行」。实机证明这个理由站不住：

用户只想采 5 条新单，导出却带上 14 条水位**之后**（更旧）的子订单，并且**写进了表**。
根因是那句「交给判重挡掉」把导出集合的正确性外包给了判重键，而判重键实测并不总是够细
（config 配成了单列 `["订单号"]`；就算配 `订单号+尺码`，属性恒为 `Variant` 的 SKU 照样撞键，
见 §5.1）。护栏一层失效，旧行就直接落表。

改法：早停页不再整页全选，只勾水位**之前**那些更新的行。

- `_orders_before_watermark(page_orders, known)`：列表新→旧，取第一个命中 `known` 的位置
  为界，之前的即本批新单；水位自身与之后的都更旧、上次已处理。纯函数、离线可测。
- `select_rows_by_order_no(page, order_nos)`：按订单号定位 `tr`，点其行首复选框的**可见
  label**（行首 input 同样是 0×0+opacity:0，直接点 input 不会触发 React 的 onChange）。
  返回勾中行数，与期望比对；找不到复选框的行只告警不中止。
- 顺序调整：`read_page_orders` 必须挪到勾选**之前**（原先是先勾再读），否则拿不到判定结果。
- 未触发早停的页照旧整页全选 —— 那些页本来全是新单，逐行勾是白费。
- 水位恰在首行（本批无新单）→ 一行都不勾，也不退回整页全选。

「同订单新增尺码」这个原始顾虑仍被覆盖：勾选按**订单号**匹配，命中订单的所有子订单行会
一起勾上。

实测（2026-08-06 那批）：本页 20 个订单 → 只勾 5 个，用户点名不要的 14 条子订单全部落在
不勾的 15 个里。
