...
产品轮播图在 1688、拼多多、Temu、Amazon 四条管线中共用 `stages/carousel.py`：
保留默认勾选图，展开全部候选，按尺码/尺寸、规格材质、产品介绍的优先级补选信息图，
总数遵守店小秘 3–10 张限制。信息图转方图时补白保留完整内容，不裁掉尺寸和说明；
仅对完全相同的文件去重，避免把版式相近但参数不同的图漏掉。

默认图和补选图均下载页面实际图片逐张英化质检，不复用可能已被本地修图覆盖的旧结论。
未通过时调用生图模型一次，上传并替换勾选后，再下载页面中的替换图复检一次。
复检未通过、取图/识别/上传/勾选失败或关键信息图超出上限时，停止该商品发布并提示人工处理，
保留现场；不会继续反复生图，也不会把无法质检的图片视为通过。
...
玩具商品的描述长图最多保留 10 张（2026-09-14）：⑬ 对超量图片先通过视觉识别品类，
玩具在合规筛选后按销售表达价值排序取前 10 张，优先覆盖尺寸、功能介绍和玩法，再考虑
配件、材质细节及使用场景，重复外观图靠后。选中的图片保持原页面展示顺序，英化和中文
复核只处理入选图片；预热同样执行此限制。取不到的图片保守保留并占用名额，无法完成
视觉筛选时不按源顺序盲删。删除后和保存后都核验实际数量，超过 10 张不进入发布。
其他品类不启用此数量限制；玩具尺寸示意图即使已有实测尺寸，也须英化保留。
