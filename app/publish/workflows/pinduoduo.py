"""拼多多商品 → 店小秘发布：拼多多属性解释规则。"""

import re

from .base import Stage, product_fields

AGE_ATTR_KEYS = ("适用年龄", "适用人群", "适合年龄段", "适合身高")
...
def normalize_size(value):
    ...
    return norm_size(value, unwrap=True, diameter=True)
...
def clean_fiber(raw):
    if not raw:
        return ""
    segments = [segment.strip() for segment in re.split(r"[/、,，]", raw)
                if segment.strip()]
...
def parse_composition(attrs):
    ...
    return _compose_from(
        clean_fiber(attrs.get("面料/材质") or attrs.get("材质") or ""),
        attrs.get("成分含量") or "",
    )
...
async def publish_one(session, task, store, **kwargs):
    ...
    return await get_workflow("pdd").publish_one(session, task, store, **kwargs)


async def run_batch(tasks, **kwargs):
    from . import get_workflow

    return await get_workflow("pdd").run_batch(tasks, **kwargs)
...
def build_stages():
    ...
    return (
        Stage("extract", "① 拼多多采集提炼", extracting._st_extract),
        Stage("claim", "② 拼多多商品认领", form._st_claim),
