            ...
            for i, u in enumerate(imgs, 1):
                ...
                try:
                    ...
                    if prefix == "main":
                        ...
                        # 【服装类图片尺寸预检】平台要求 ≥1340×1785px，这里只打标不报警
                        # （标记被阶段⑦ 用来把不达标图排到补图候选末尾，见
                        # stages.skc._pad_row_images）。
                        # 【为什么不逐张 warning，改成收尾汇总一行】1688 取的是
                        # offerImgList 原图、没有降采样，商家原图普遍 800×800 / 1000×1000，
                        # 命中率接近 100%——每单刷十几行黄字；而 ⑥素材图 / ⑦SKC /
                        # ⑦b 预览图上传前都会先做合规化（square_image / batch_fit34），
                        # 真正把关尺寸的是上传闸门 upload_image。逐张报既不可行动、又把
                        # 真问题淹没在黄字里。下限取自 images，与上传闸门同源，避免两处漂移。
                        w, h = dims.get("w", 0), dims.get("h", 0)
                        if w > 0 and h > 0 and (w < images.CLOTH_MIN_W or h < images.CLOTH_MIN_H):
                            entry["sizeWarning"] = (f"尺寸 {w}×{h} 不达标（服装类要求 "
                                                    f"≥{images.CLOTH_MIN_W}×{images.CLOTH_MIN_H}）")
                    downloaded[prefix].append(entry)
                except Exception as e:
                    downloaded[prefix].append({"url": u, "error": str(e)})
                    logger.warning(f"{prefix}-{i:02d} 下载失败：{e}")
        # 源图偏小只汇总报一次（理由见上面 sizeWarning 的注释）：后续 ⑥⑦⑦b 会把它们
        # 统一放大到达标，这里报出来是让人知道「本单出图是放大的」，不必逐张刷 warning
        low = [e for e in downloaded["main"] if e.get("sizeWarning")]
        if low:
            logger.info(
                f"主图 {len(low)}/{len(downloaded['main'])} 张低于服装类 "
                f"{images.CLOTH_MIN_W}×{images.CLOTH_MIN_H}，将由 ⑥⑦⑦b 的合规化统一放大")
...
3. sizeMeasurements —— 尺码的【成品实测尺寸】表（服装是衣长/胸围/袖长/裤长等；
   ...
   【平铺半围要留住标记】表头写「腰围x2」「臀围×2」「腰围半围」「平铺腰围」「腰围单面」
   「腰围1/2」、格子里写「20x2」时，说的都是同一件事：「这一列是平铺单面的半围，乘 2
   才是绕一圈的全围」。此时参数名【原样照抄图里的写法】（"腰围x2"、"平铺腰围"、
   "腰围半围"），值只填乘号前那个数（20）——不要丢掉标记（丢掉「腰围x2」的标记就成了
...
1. sizeMeasurements —— 各尺码的【平铺实测尺寸】（衣长/胸围/袖长/肩宽/裤长等，单位 cm）：
   ...
   - 文字里写「腰围x2 20x2」「腰围半围」「平铺腰围」这类【平铺半围】写法时，参数名
     原样带上标记（"腰围x2"、"平铺腰围"）、值只填乘号前的数（20）：不要丢掉标记，
     也不要自己乘 2 或填算好的 40，下游按平台参数要的量法换算。
