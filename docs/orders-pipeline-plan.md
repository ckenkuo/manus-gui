...
- 云端成本表与订单表共用 `KdocsSheet._read_columns()`。读取组合键时，合并单元格的值
  必须按接口返回的 `rowFrom..rowTo` 展开，再与每行规格配对；只读合并区域首行会漏掉
  下方已登记的规格。普通空白格不能向下填充，展开范围不能越过表头或扫描上限。
- 成本表按用户确认的口径只用 `SPU ID` 判重：表内已有该 SPU，全部规格都跳过；
  新 SPU 每批只写一行。订单登记表仍使用原有订单号、子订单号等判重规则。
- 订单下一页点击遇到 `intercepts pointer events` 时，先清理站点弹窗及已知插件浮层，
  再重试一次；每次点击用最多 3 秒的探测窗口，点击阶段共享 `timeout_ms` 的总预算。
  普通加载、可见性和稳定性超时在预算内继续等待，不因单次 3 秒超时中止。
  持续遮挡才对该分页按钮执行 DOM 点击，并检查按钮可见、未禁用、页码未先行变化；
  其它点击超时不走 DOM 降级路径，预算耗尽后报错。
- 点击后必须确认每页条数不变且页码恰好增加 1；未变化、跳页或分页状态异常均中止，
  不把重复页或漏页当作成功。恢复过程不屏蔽未知浮层，也不自动修改或删除线上表格数据。
