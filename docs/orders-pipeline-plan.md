...
`manus输出/订单采购汇总/新增订单采购汇总_时间戳.xlsx`。汇总表包含采购总件数、订单数、
...
**水位＝目标 Sheet「订单号」列的已登记值集合**（`read_known_order_nos`，复用
`existing_key_values`，一次 zip 解压）。不用「最新一条订单号」当水位：订单号不单调
（`PO-045-`/`PO-159-`/`PO-211-` 是站点前缀，不是序号），没法比大小；也不用「最新创建
时间」：历史 368 行里 208 行的「平台创建时间」是空的、时间文本还有 4 种长度（见 §8），
不可靠。集合成员判定是精确的，且这批数据本来就要读来判重。
...
**早停判据**（`should_stop_incremental`，纯函数、离线可测）：本页订单号全部已登记，且本批
此前至少见过一条新单。首次跑水位为空集，第一页全是新单，自然翻到底。

**判定为全已知的那一页照样勾选导出**：判重键是 `订单号 + 尺码`，订单号已登记不代表它每个
尺码都登记了（同订单新增尺码是真实情况），跳过就漏行。重叠的旧单由判重挡掉，代价只是多
导出几十行。

**两条护栏**（早停依赖列表是新→旧排序，`list_url` 自带 `sortType=1`，但页面手点列头排序
或配置里 URL 被改都会破掉它）：

- **A 交错检测（主力，确定性）**：新→旧排序下一旦出现已登记单，它后面不该再有新单。出现
  交错就不早停、翻完全量、告警。不依赖任何时间字段。它还顺带覆盖「表里有空洞」——上批某几
  条没登记成功时也会触发，正好翻完把洞补上。
- **B 创建时间单调校验（best-effort）**：`check_created_desc` 校验跨页/页内时间不递增。
  ⚠️ 列表页 DOM 里有没有创建时间**未经实机确认**，读不到就返回空串、静默降级到只用 A，
  不中断也不阻塞早停。实机跑一次即可确认 B 是否生效。
